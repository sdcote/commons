...
/**
 * A class designed to add structure to repeatable logic.
 *
 * <p>The primary use case for this class is to perform a single task
 * {@code doWork()} repeatedly until terminated. This allows for such tasks as
 * file system maintenance, log rotation, and anything that needs to be
 * performed repeatedly until specifically told to stop processing.</p>
 *
 * <p>This class provides a basic life cycle that includes initialization,
 * execution, and termination. When this class is run, it calls the
 * {@code initialize()} method to perform any necessary setup before starting
 * the execution loop. Once the execution loop is complete, the
 * {@code terminate()} method is called to perform any necessary cleanup before
 * exiting.</p>
 *
 * <p>If the restart flag is set to true, the ThreadJob remains active and
 * reenters the run loop where it is re-initialized. This allows for the
 * altering of the ThreadJob's configuration and restarting the job with the
 * new configuration.</p>
 *
 * <p>The heart of the ThreadJob class is the {@code doWork()} method that
 * contains the logic to be repeatedly performed. Subclasses of this class
 * define the specific behavior to be executed in each iteration of the
 * execution loop. While the {@code doWork()} method is used in the ThreadJob
 * to run whatever {@code Runnable} object is placed there, the
 * {@code doWork()} is often overridden by subclasses to provide custom
 * behavior.</p>
 *
 * <p>The run loop can be paused by calling {@code suspend()} and later resumed
 * with {@code resume()}. Note that if the run loop is suspended, the
 * {@code doWork()} method will not be called until {@code resume()} is called.
 * Additionally, execution will not be suspended until {@code doWork()} has
 * returned. This suspension is only for the run loop, not the thread itself.
 * </p>
 *
 ...
 * general, if the class may need to run in a multithreaded environment, this
...
public class ThreadJob implements Runnable {
  ...
  /** Indicates that we should shut down, reinitialize, and start running again */
  protected volatile boolean restart = false;

  /** Indicates we have been asked to temporally stop processing */
  protected volatile boolean suspended;

  /** The flag that represents our idle status */
   ...
   * terminates. Only a {@code user} thread will keep the JRE running and this
   * method creates a {@code user} thread. </p>
   *
   ...
   * thread as "daemon" threads will be terminated automatically when the JRE 
   * terminates. Only a {@code user} thread will keep the JRE running and this
   * method creates a {@code user} thread. The naming of the method is to
   * characterize the "background process" nature of a daemon.</p>
   *
   ...
   * <p><strong>Note</strong> that this does not create a Java {@code daemon} 
   * thread as "daemon" threads will be terminated automatically when the JRE 
   * terminates. Only a {@code user} thread will keep the JRE running.</p>
   *
  ...
  /**
   * Return whether the thread has entered and is currently within the
   * main run loop.
   *
  ...
  /**
   * Indicates if this thread job has started processing.
   *
  ...
  /**
   * Called by the {@code run()} method when it enters and exits the main run loop.
   *
   ...
   *
   * <p>This method is called again with false just before exiting the run()
  ...
  public void shutdown() {
    synchronized (mutex) {
      ...
      if (current_thread != null) {
        ...
        // Make sure suspended threads are resumed so they can shut down
        mutex.notifyAll();
      }
    }
  }
  ...
  /**
   * @return whether the current thread is set to shut down.
   */
  ...
  /**
   * @return whether the current thread is set to restart.
   */
  ...
  /**
   * @return whether the current thread is in a suspended state.
   */
  ...
  /**
   * @return whether the current thread is in an idle state.
   */
  ...
  /**
   * @return whether the current thread is hyper (never idles).
   */
  ...
  /**
   * Perform any subclass initialization.
   * 
  ...
  /**
   * Final resource cleanup before exiting or restarting.
   *
  ...
  /**
   * Set whether we are in an idle state.
   * 
  ...
  public void run() {
    ...
    do {
      ...
      // Set up everything we need to run
      initialize();
      ...
      while (!isShutdown()) {
        // Do whatever the subclass wants
        doWork();
   ...
   * actual thread returned by the getThread() method call.</p>
   *
   * @see #getThread
   */
  public void join() {
   ...
   * out of an idle state, it is sometimes desirable to keep the thread active
   * if the logic expects to perform more work that should not be interrupted.
  ...
  /**
   * Returns the parking strategy of the thread being used.
   *
  ...
  /**
   * Sets the parking strategy of the thread.
   *
   * <p>In most VMs, the most efficient way to suspend execution is to use the
   * wait() method. Some VMs use fewer CPU cycles if the sleep() method is used.
