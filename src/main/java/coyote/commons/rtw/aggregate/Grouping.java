 ...
 * if each product price appeared together as a group, so when a new
 ...
 *
 * <p>The aggregator can limit the number of frames it returns
...
public class Grouping extends AbstractFrameAggregator implements FrameAggregator {
    ...
    private final Map<String, List<DataFrame>> dataMap = new HashMap<>();
     ...
     */
    private void sortKeys(RTW.Sort mode) {
        if (mode != RTW.Sort.NONE) {
            if (mode == RTW.Sort.ASCEND || mode == RTW.Sort.DESCEND) {
                Collections.sort(keys);
            } else {
                Collections.sort(keys, String.CASE_INSENSITIVE_ORDER);
            }
            if (mode == RTW.Sort.DESCEND || mode == RTW.Sort.DESCEND_CI) {
