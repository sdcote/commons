...
import coyote.commons.rtw.db.FrameStore;
  ...
  private void init() {
    ...
    if (context instanceof TransformContext) {
      ...
      if (connection == null) {
        ...
        if (getConnector() == null) {
          ...
          try {
            ...
            if (Log.isLogging(Log.DEBUG_EVENTS)) {
              Log.debug(String.format("Writer.using_target", getClass().getName(), database.getTarget()));
              Log.debug(String.format("Writer.using_driver", getClass().getName(), database.getDriver()));
              Log.debug(String.format("Writer.using_library", getClass().getName(), database.getLibrary()));
              Log.debug(String.format("Writer.using_user", getClass().getName(), database.getUserName()));
              Log.debug(String.format("Writer.using_password", getClass().getName(), StringUtil.isBlank(database.getPassword()) ? 0 : database.getPassword().length()));
            }
        ...
        Log.debug(String.format("Writer.using_existing_connection", getClass().getName()));
      }
  ...
  protected synchronized Database getDatabase() {
    ...
    if (retval == null) {
      ...
      if (retval == null && getConfiguration().containsIgnoreCase(getDriver())) {
        ...
        database = retval;
        Log.debug(String.format("Component.using_local_database_definition", database.toString()));
      }
    }
    return retval;
  }
  ...
  private Connection getConnection() {
    ...
    if (connection == null) {
      ...
      try {
        ...
        if (connection != null) {
          Log.debug(String.format("Writer.connected_to", getClass().getName(), getTarget()));
          ...
          Log.debug(String.format("Writer.connected_to_product", getClass().getName(), meta.getDatabaseProductName(), meta.getDatabaseProductVersion(), meta.getDatabaseMajorVersion(), meta.getDatabaseMinorVersion()));
        } else {
  ...
  private boolean checkTable() {
    ...
    if (!DatabaseUtil.tableExists(table, getConnection())) {
      ...
      if (isAutoCreate()) {
        ...
        Log.debug(String.format("Writer.creating_table", getClass().getName(), table, command));
