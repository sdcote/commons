...
public abstract class AbstractFrameFileWriter extends AbstractFrameWriter implements FrameWriter {
  ...
  public void setConfiguration(Config cfg) throws ConfigurationException {
    ...
    if (cfg.contains(ConfigTag.APPEND)) {
      try {
        ...
        Log.info("Writer.append_flag_is_not_valid " + cfg.getAsString(ConfigTag.APPEND));
      }
    }
    Log.debug("Writer.append_flag_is_set_as"+ isAppending());
  }
  ...
  public void open(final TransformContext context) {
      ...
      if (printwriter == null) {
          ...
          Log.debug( "Writer.using_target"+ getClass().getSimpleName()+ target);
          ...
          if (StringUtil.equalsIgnoreCase(STDOUT, target)) {
              ...
              if (UriUtil.isFile(uri)) {
                  ...
                  if (targetFile == null) {
                      Log.warn(String.format("Writer.The target '{%s}' does not represent a file", target));
                  }
              } else {
              ...
              Log.debug("Writer.using_target_file"+ getClass().getSimpleName()+ targetFile.getAbsolutePath());
