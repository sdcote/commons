...
public abstract class WSD extends HTTPD {
   ...
   * logger to log to.
   */
  private static final Logger LOG = Logger.getLogger( WSD.class.getName() );
  ...
  public static String makeAcceptKey( String key ) throws NoSuchAlgorithmException {
    ...
    String text = key + WSD.WEBSOCKET_KEY_MAGIC;
  ...
  public WSD( String hostname, int port ) {
    super( hostname, port );
  }
  ...
  private boolean isWebSocketConnectionHeader( Map<String, String> headers ) {
    String connection = headers.get( WSD.HEADER_CONNECTION );
  ...
  protected boolean isWebsocketRequested( IHTTPSession session ) {
    ...
    String upgrade = headers.get( WSD.HEADER_UPGRADE );
    boolean isCorrectConnection = isWebSocketConnectionHeader( headers );
    boolean isUpgrade = WSD.HEADER_UPGRADE_VALUE.equalsIgnoreCase( upgrade );
