...
public class Tabs {
    ...
    private static String FIXTUREID = UUID.randomUUID().toString().toLowerCase();
    private static String FIXTUREGRP = null;
    private static Tabs fixture;
    private static File workDirectory;
    private static File logDirectory;
    private static final boolean checkedClasspathAlready = false;
    /**
    ...
    private static final HashMap shadowClasses = new HashMap();
    private static final ArrayList missingLibraries = new ArrayList();
    ...
    public static Iterator getArmIterator() {
        ...
        synchronized (Tabs.armMasters) {
            for (final Iterator it = Tabs.armMasters.values().iterator(); it.hasNext(); list.add(it.next())) {
            }
        }
    ...
    public static Iterator getCounterIterator() {
        final ArrayList list = new ArrayList();
        for (final Iterator it = Tabs.counters.values().iterator(); it.hasNext(); list.add(it.next())) {
        }
        return list.iterator();
    }
    ...
    public static float getFreeMemoryPercentage() {
        return (float) Tabs.getFreeMemory() / (float) Tabs.getMaxHeapSize();
    }
    ...
    public static Iterator getGaugeIterator() {
        final ArrayList list = new ArrayList();
        synchronized (Tabs.gauges) {
            for (final Iterator it = Tabs.gauges.values().iterator(); it.hasNext(); list.add(it.next())) {
            }
        }
        return list.iterator();
    }
    ...
    public static float getHeapPercentage() {
        return 1 - ((float) Tabs.getAvailableMemory() / (float) Tabs.getMaxHeapSize()); // percent of max
    }
    ...
    public static Iterator getStateIterator() {
        final ArrayList list = new ArrayList();
        for (final Iterator it = Tabs.states.values().iterator(); it.hasNext(); list.add(it.next())) {
        }
        return list.iterator();
    }
    ...
    public static Iterator getTimerIterator() {
        final ArrayList list = new ArrayList();
        synchronized (Tabs.masterTimers) {
            for (final Iterator it = Tabs.masterTimers.values().iterator(); it.hasNext(); list.add(it.next())) {
            }
        }
        return list.iterator();
    }
    ...
    public static float getUsedMemoryPercentage() {
        return 1 - ((float) Tabs.getFreeMemory() / (float) Tabs.getMaxHeapSize()); // percent of max
    }
