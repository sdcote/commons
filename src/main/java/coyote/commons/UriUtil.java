...
public class UriUtil {
    ...
    private static void addPort(final String scheme, final int port) {
        UriUtil.portMap.put(scheme.toLowerCase(), Integer.valueOf(port));
    }
    ...
    private static void addUdpPort(final String scheme, final int port) {
        UriUtil.udpMap.put(scheme.toLowerCase(), Integer.valueOf(port));
    }
    ...
    public static File getFile(final URL url) {
        try {
            ...
            System.err.println("Could not convert '" + url + "' to a URI");
            e.printStackTrace();
        }
