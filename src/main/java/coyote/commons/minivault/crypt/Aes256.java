 ...
 * <p>The specification in &quot;Federal Information Processing Standards Publication 197&quot; defines
 ...
 * <li>256 bit.</li>
 * </ul>
 * To make this implementation more compact, only the strongest version is implemented.</p>
 *
 * <p>The block size is 128 bit.</p>
 */
public final class Aes256 {
   ...
   *
   * <p>The encryption and decryption will use the expanded key.</p>
   *
   ...
   *
   * @param value array in which the first {@code WORD_SIZE} {@code byte}s will be substituted. This array will be modified.
  ...
  /**
   * Rotate the {@code byte}'s in a word.
   *
   * <p>The {@code byte}'s will be cycled left by one {@code byte}. The modification will be in place, so the original
   * argument is changed after the method invocation.
   *
   * @param value Array in which the first {@code WORD_SIZE} {@code byte}'s will be changed due to the rotation. The
   *              contents of this array is changed by this invocation.
   */
  ...
  /**
   * The polynomial represented by {@code b} will be multiplied by its free variable.
   *
   * <p>This multiplication takes place in a finite field. The resulting polynomial can still be represented in one
   * {@code byte}.</p>
   *
   * <p> The bits {@code 0} to {@code 7} are the coefficients of the powers {@code x} to {@code x**8}. </p>
   *
  ...
  /**
   * Two polynomial will be multiplied with each other.
   *
   * <p>The representation of the polynomial is described in {@link Aes256#times2}.</p>
   *
   * <p>The multiplication will be performed by successive invocations of {@link Aes256#times2}.</p>
   *
   ...
   * Encrypts one block.
   *
   * <p>The input block lies in {@code inBlock} starting at the position {@code inIndex}. The {@code inBlock} won't be
   * modified by this method. The encrypted block will be stored in {@code outBlock} starting at position
   * {@code outIndex}.</p>
   *
  ...
  /**
   * Changes all {@code byte}s of the state. This method is the inverse of {@link Aes256#shiftRows}.
   */
