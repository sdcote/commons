 ...
 *
 * <p>Currently Google S2 is used to retrieve favicons.</p>
 */
public class IconStorage {
  ...
  private IconStorage() {
    enabled = MiniVault.USER_PREFERENCE.is("fetch.favicons.enabled", false);
  ...
  public synchronized ImageIcon getIcon(String url) {
    ...
    String iconFileName;
    try {
      iconFileName = StringUtil.byteArrayToHex(CryptUtils.getSha256Hash(domain.toCharArray())) + ".png";
    } catch (Exception e) {
      return DEFAULT_ICON;
    }
  ...
  private ImageIcon getAndCacheIcon(String domain, String iconFileName) {
    ...
    try {
      ...
      // We put a standard icon to cache.Note that on the next application run we will try to retrieve the icon again,
