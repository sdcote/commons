...
import static coyote.commons.StringUtil.isBlank;
 ...
 * categories remain unaffected. This allows you to turn on and off all
 ...
 * WARN, ERROR and FATAL events to {@code System.out} through the ConsoleAppender
 * logging class.</p>
 */
final class LogKernel {
     ...
     */
    static final Hashtable<String, Long> stringToCode = new Hashtable<>();
    /**
     * Map of all the category names keyed by their category code.
     */
    static final Hashtable<Long, String> codeToString = new Hashtable<>();
    /**
     * Map of all the loggers in the fixture keyed by their name.
     */
    static final Hashtable<String, Logger> nameToLogger = new Hashtable<>();
    static final long started = System.currentTimeMillis();
    /**
     * If a loggers name appears in this list, it will NOT be removed from the
     * logger collection.
     */
    private static final HashSet<String> permanentLoggers = new HashSet<>();
    static long masks; // union of masks of all loggers

    static {
        // Set up a shutdown hook to clean up the loggers
        try {
     ...
     */
    public static synchronized void addLogger(final String name, final Logger logger) {
        if (isBlank(name))
     ...
     */
    public static synchronized void append(final long code, final Object event, final Throwable cause) {
     ...
     * @param event    The event to log.
     */
    public static synchronized void append(final String category, final Object event) {
        LogKernel.append(category, event, null);
    }
     ...
     */
    public static synchronized void append(final String category, final Object event, final Throwable cause) {
        LogKernel.append(LogKernel.getCode(category), event, cause);
    }
     ...
     */
    public static synchronized String getCategory(final long code) {
        return LogKernel.codeToString.get(code);
    }
     ...
     * @see #getCode(String)
     */
    public static synchronized String[] getCategoryNames() {
     ...
     */
    public static synchronized int getLoggerCount() {
     ...
     */
    public static synchronized List<String> getLoggerNames() {
     ...
     * @return an enumeration over all the current loggers.
     */
    public static synchronized List<Logger> getLoggers() {
     ...
     * operations.</p>
     */
    public static synchronized void removeDefaultLogger() {
     ...
     * @param category The category.
     */
    public static synchronized void startLogging(final String category) {
        Logger lgr = null;
        for (final Enumeration<Logger> en = LogKernel.nameToLogger.elements(); en.hasMoreElements(); ) {
            lgr = en.nextElement();
            if (!lgr.isLocked()) {
     ...
     * @param category The category.
     */
    public static synchronized void stopLogging(final String category) {
        Logger lgr = null;
        for (final Enumeration<Logger> en = LogKernel.nameToLogger.elements(); en.hasMoreElements(); ) {
            lgr = en.nextElement();
            if (!lgr.isLocked()) {
