...
public class TestHttpServer extends AbstractTestHttpServer {
    ...
    public void doArgumentTest() throws InterruptedException, IOException {
        ...
        try {
            ...
            assertTrue( str.indexOf("data") >= 0,"The response entity didn't contain the string 'data'");
        } finally {
            if (response != null)
                response.close();
        }
    ...
    public void testIfNoneMatchHeader() throws IOException {
        ...
        try {
            ...
            CloseableHttpClient httpClient = HttpClients.createDefault();
            response = httpClient.execute(httpGet);
            assertEquals(304, response.getStatusLine().getStatusCode(), "The response status to a reqeuest with 'if-non-match=*' header should be NOT_MODIFIED(304), if the file exists");
        } finally {
            if (response != null) {
                response.close();
            }
    ...
    public void testRangeHeaderAndIfNoneMatchHeader() throws IOException {
        ...
        try {
            ...
            CloseableHttpClient httpClient = HttpClients.createDefault();
            response = httpClient.execute(httpGet);
            assertEquals(304, response.getStatusLine().getStatusCode(), "The response status to a request with 'if-non-match=*' header and 'range' header should be NOT_MODIFIED(304)," + " if the file exists, because 'if-non-match' header should be given priority");
        } finally {
            if (response != null) {
                response.close();
            }
    ...
    public void testURLContainsParentDirectory() throws IOException {
        ...
        try {
            ...
            response = httpClient.execute(httpGet);
            assertEquals(403, response.getStatusLine().getStatusCode(), "The response status should be 403(Forbidden), " + "since the server won't serve requests with '../' due to security reasons");
        } finally {
            if (response != null) {
                response.close();
            }
