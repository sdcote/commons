...
public class JdbcTemplateTest {
    ...
    public void queryTests() {
        ...
        List<Employee> emps = template.query("SELECT * FROM EMPLOYEE OFFSET ? ROWS FETCH NEXT ? ROWS ONLY", Arrays.<Object>asList(0, 5), employeeDataMapper);
        // System.out.println( "Found " + emps.size() + " records" );
        assertEquals(2, emps.size());

        emps = template.query("SELECT * FROM EMPLOYEE WHERE emp_no= ?;", Collections.<Object>singletonList(1001), employeeDataMapper);
        // System.out.println( "Found " + emps.size() + " records" );
